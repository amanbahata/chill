...
public class MostPlayedMoviesPresenterTest {

    private static final String TEST_ERROR = "error";

    @Mock
    private MostPlayedMoviesFragment view;
    @Mock private IDataManager dataManager;
    @Mock private Consumer<List<MostPlayedMovies>> successConsumer;
    @Mock private Consumer<Throwable> failConsumer;

    private MostPlayedMoviesPresenter<MostPlayedMoviesFragment> presenter;
    ...
    public void setUp() throws Exception {
        ...
        presenter = new MostPlayedMoviesPresenter<>(dataManager, new TestSchedulerProvider(), new CompositeDisposable());
    ...
    @Test
    public void test_onCallMostPlayedMoviesList_Success() throws Exception {

        List<MostPlayedMovies> item = new LinkedList<>();

        Observable<List<MostPlayedMovies>> observable = Observable.just(item);
        observable.subscribe(successConsumer, failConsumer);
        when(this.dataManager.getMostPlayedMovies()).thenReturn(observable);

        presenter.onCallMostPlayedMoviesList();
    ...
    public void test_onCallTrendingNewsList_Failure() throws Exception {
        ...
        Observable<List<MostPlayedMovies>> observable = Observable.error(exception);
        observable.subscribe(successConsumer, failConsumer);
        when(this.dataManager.getMostPlayedMovies()).thenReturn(observable);
